...
function linkSymlinks(symlinks, siteId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const symlink of symlinks) {
            ...
            try {
                const stats = yield fs.lstat(path.join(site.paths.webRoot, symlink.dest));
            ...
            try {
                const result = yield fs.symlink(symlink.source, path.join(site.paths.webRoot, symlink.dest), 'dir');
                if (result) {
                    errors.push(result);
                }
            }
...
function default_1(context) {
    ...
    ipcMain.on(constants_1.IPC_EVENTS.SAVE_SITE_SYMLINKS, (event, siteId, symlinks) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            LocalMain.getServiceContainer().cradle.localLogger.log('error', `Failed saving symlinks for site ${siteId}.`);
            LocalMain.sendIPCEvent(constants_1.IPC_EVENTS.SAVE_SITE_SYMLINKS_FAILURE, [
                err,
            ]);
            return;
        }
        const errors = yield linkSymlinks(symlinks, siteId);
        if (errors && errors.length > 0) {
            dataService.updateSite(siteId, {
                id: siteId,
                symlinks: originalSymlinks,
            });
            LocalMain.sendIPCEvent(constants_1.IPC_EVENTS.SAVE_SITE_SYMLINKS_FAILURE, errors);
        }
        else {
            LocalMain.sendIPCEvent(constants_1.IPC_EVENTS.SAVE_SITE_SYMLINKS_SUCCESS);
        }
    }));
    ipcMain.on(constants_1.IPC_EVENTS.SHOW_ERROR_DIALOG, (event, title, message) => {
    ...
    LocalMain.HooksMain.addFilter('modifyAddSiteObjectBeforeCreation', (site, newSiteInfo) => {
        ...
        if (preferences.useDefaultSymlinks &&
            ...
            site.symlinks = preferences.symlinks.filter((symlink) => symlink.enabled);
        }
        return site;
    });
